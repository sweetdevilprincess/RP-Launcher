#!/usr/bin/env python3
"""🚧 WIP LAUNCHER 🚧 - Testing new features without affecting production."""
...
def print_wip_banner():
    """Print a clear WIP mode banner."""
    print("\n" + "=" * 70)
    print("🚧" * 35)
    print("=" * 70)
    print("              🚧 WIP MODE - TESTING ENVIRONMENT 🚧")
    print("=" * 70)
    print("  This launcher uses DEVELOPMENT code from the 'wip/' folder")
    print("  Your production RP system is NOT affected by this launcher")
    print("=" * 70)
    print("🚧" * 35)
    print("=" * 70 + "\n")


def start_wip_bridge(rp_dir: Path, background: bool = False) -> subprocess.Popen:
    """Start the WIP bridge (from wip/ folder) in a separate process.
    ...
    # 🚧 KEY DIFFERENCE: Use wip/tui_bridge.py instead of src/tui_bridge.py
    bridge_script = base_dir / "wip" / "tui_bridge.py"
    python_exe = sys.executable

    if not bridge_script.exists():
        print(f"❌ ERROR: WIP bridge not found at: {bridge_script}")
        print("   Make sure wip/tui_bridge.py exists!")
        input("\nPress Enter to exit...")
        sys.exit(1)

    print(f"🚧 Starting WIP bridge for: {rp_dir.name}")
    print(f"   Using: {bridge_script}")
    ...
    if background:
        ...
        print("🚧 WIP Bridge started in background (hidden)")
    else:
        # Debug mode - new visible terminal window
        if sys.platform == 'win32':
            # On Windows, create a new console window with custom title
            process = subprocess.Popen(
                [python_exe, str(bridge_script), rp_dir.name],
                cwd=str(base_dir),
                creationflags=subprocess.CREATE_NEW_CONSOLE
            )
            print("🚧 WIP Bridge started in new terminal window")
            print("   Look for: '🚧 WIP BRIDGE' in the window title")
        else:
...
def select_rp_folder(rp_folders):
    ...
    print("\n🚧 Available RP folders for WIP testing:")
...
def main():
    ...
    if sys.platform == 'win32':
        try:
            os.system('title 🚧 WIP RP Launcher - Testing Mode')
        except:
            pass
    ...
    try:
        ...
        if len(sys.argv) >= 2 and not sys.argv[1].startswith("--"):
            ...
            if len(rp_folders) == 1:
                ...
                rp_dir = rp_folders[0]
                print(f"🚧 Auto-selected RP folder for WIP testing: {rp_dir.name}")
            else:
                # Let user choose
                rp_dir = select_rp_folder(rp_folders)
                print(f"🚧 Selected for WIP testing: {rp_dir.name}")

        # Register cleanup function
        atexit.register(cleanup_bridge)

        # Start the WIP bridge
        print("\n" + "=" * 70)
        _bridge_process = start_wip_bridge(rp_dir, background=background_mode)
        print("=" * 70)

        # Run the TUI (production TUI - we haven't changed it)
        print(f"\n🚧 Launching RP Client TUI (WIP Mode) for: {rp_dir.name}")
        print("=" * 70)
        print("   Using: PRODUCTION TUI (src/rp_client_tui.py)")
        print("   Using: WIP BRIDGE (wip/tui_bridge.py) ✨")
        print("=" * 70 + "\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        import traceback
        print("\n" + "=" * 70)
        print("🚧 UNCAUGHT EXCEPTION IN WIP LAUNCHER:")
        traceback.print_exc()
        print("=" * 70)
        stop_bridge(_bridge_process)
        input("\nPress Enter to exit...")
